...
SPEED_SCALE      = 0.012
DODGE_RADIUS     = 0.5      # triggered dodge mechanism when in radisu
ARRIVAL_DIST     = 0.08
MISSION_DIST     = 1.8
WORLD_SIZE       = 10.0
OCTREE_CAPACITY  = 8        # max drone per octree nodes
...
def brute_force_collisions(positions: np.ndarray, active: np.ndarray, radius: float):
    """
    Returns (repulsion array, dodging boolean mask).
    Pure NumPy — vectorised over the active set.
    """
    n = len(positions)
...
class OctreeNode:
    ...
    def __init__(self, cx: float, cy: float, cz: float, hs: float):
        ...
        self.hs       = hs
        self.indices  = []          # drone indices stored at this leaf
        self.children = None        # list[OctreeNode] when subdivided
        self.is_leaf  = True

    def contains(self, px, py, pz) -> bool:
        return (
            self.cx - self.hs <= px <= self.cx + self.hs and
            self.cy - self.hs <= py <= self.cy + self.hs and
            self.cz - self.hs <= pz <= self.cz + self.hs
        )
    ...
    # query radius to check for collisions
...
def octree_collisions(positions: np.ndarray, active: np.ndarray,
                      radius: float, root: OctreeNode):
    n = len(positions)
    repulsion  = np.zeros_like(positions)
    is_dodging = np.zeros(n, dtype=bool)
    checked    = set()
...
def step_physics(positions, targets, status, repulsion, is_dodging):
    ...
    move   = np.where(active[:, None],
                      diff / (dist + 1e-9) * SPEED_SCALE,
                      0.0)
    ...
    status[(status != 1) & (arrived_dist < ARRIVAL_DIST)] = 1  # arrived (grean)
    status[(status != 1) & is_dodging]                    = 2  # dodging  (red)
    status[(status != 1) & ~is_dodging]                   = 0  # cruising (cyan)
...
def run_simulation(mode: str, num_drones: int):
    ...
    box = pv.Cube(center=(WORLD_SIZE/2,)*3,
                  x_length=WORLD_SIZE, y_length=WORLD_SIZE, z_length=WORLD_SIZE)
    plotter.add_mesh(box, style="wireframe", color="white", opacity=0.15, line_width=1)

  
    plane = pv.Plane(center=(WORLD_SIZE/2, WORLD_SIZE/2, 0),
                     direction=(0, 0, 1),
                     i_size=WORLD_SIZE, j_size=WORLD_SIZE,
    ...
    plotter.add_mesh(target_cloud, color="orange", point_size=5,
                     render_points_as_spheres=True, opacity=0.4)

    octree_actors = []

    # HUD
    mode_label = "BRUTE FORCE  O(n²)" if mode == "brute" else "OCTREE  O(n log n)"
    hud = plotter.add_text(
        "", position="upper_left", color="white", font_size=11, shadow=True
    )
    plotter.add_text(
        f"Mode: {mode_label}", position="lower_left",
        color="yellow", font_size=10, shadow=True
    )
        ...
        if mode == "octree" and octree_root is not None:
            ...
            for (cx, cy, cz, hs) in boxes:
                if hs < WORLD_SIZE / 4:
                    ...
                    a = plotter.add_mesh(cube, style="wireframe",
                                         color="deepskyblue", opacity=0.06, line_width=1)
...
def get_user_inputs():
    ...
    print("║  3. Benchmark (no window, prints table)  ║")
    print("╚══════════════════════════════════════════╝")
    ...
    try:
        n = int(input("Number of drones (e.g. 60): ").strip())
    except ValueError:
        n = 60
...
if __name__ == "__main__":
    ...
    if mode == "benchmark":
        frames = 120
        try:
            frames = int(input("Number of benchmark frames [120]: ").strip() or "120")
        except ValueError:
            pass
